...
- **Último hito:** Implementado y mergeado el issue #46: catálogo de formas jurídicas por jurisdicción y tipo de sujeto. Añadido `is_legal_person` a `t4_companies` (persona física/jurídica, enum `SubjectType` en core), FKs de jurisdicciones a `t1_countries`, catálogo `es3_legal_forms` con seed de 17 formas españolas según `docs/tipos-de-empresas.md`, capacidad `LegalFormsCapability` resoluble por jurisdicción y tipo de sujeto, selector de tipo de sujeto + `ComboBox` de forma jurídica en la vista de Empresas, y `created_at` como `TIMESTAMP WITH TIME ZONE`. Change archivado como `2026-08-07-is46-legal-forms-catalog`. El concepto de sujeto y el catálogo español se documentan en `docs/Product-Spec.md` (§8.3.1 y §8.3.2).
