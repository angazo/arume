...
| `arume-uk` | Módulo británico: solo siembra datos de jurisdicción en los catálogos core (formas jurídicas y asociación país↔divisa) desde `db/migration/uk/`; no posee tablas ni capacidades fiscales todavía; se registra como `FiscalModule` sin capabilities |
...
- **Último hito:** Implementado y mergeado el issue #55: módulo de Reino Unido (`arume-uk`) y refactor del catálogo de formas jurídicas. El catálogo `t5_legal_forms` pasa a PK doble `(country_alpha2_code, code)`; `is_legal_person` se sustituye por la columna informativa `is_organization` (familia individual/organización, no parte de la PK ni de la FK), `t6_companies` usa FK `(legal_form_jurisdiction, legal_form_code)`, se elimina el enum `SubjectType` del dominio y la UI filtra por familia (`LegalFormFamily`). El Reino Unido siembra 7 formas (ST, PS, LLP, Ltd, PLC, CLG, CIC) y GB↔GBP en `t4_country_currency` desde `uk/V0.1.0.0`, sin tablas propias. Dos módulos nacionales (ES, GB) conviven con tres historiales Flyway independientes renombrados con `_` inicial (`_flyway_core_schema_history`, `_flyway_es_schema_history`, `_flyway_uk_schema_history`); el `FiscalModuleRegistry` se compone por component scan y cada módulo nacional se auto-registra (`@Component` en `SpainFiscalModule`/`UkFiscalModule`, `@Service` en `InvoiceSeriesApplicationService`), de modo que `BusinessApplicationConfiguration` solo ensambla el registro. Change archivado como `2026-08-08-is55-uk-module-legal-forms`. El refactor se documenta en `docs/Product-Spec.md` (§8.3.1 y §8.3.2). El issue #53 (backlog) sigue abierto para nombres de divisa traducidos.
- **Próximo hito:** Definir el siguiente change de lógica de negocio sobre la base internacional, previsiblemente relacionado con facturas, clientes o la selección de empresa activa; o dotar al módulo de Reino Unido de su primera capacidad fiscal (p. ej. series de facturación) ahora que el armazón de dos jurisdicciones está validado.
...
- **Base de datos**: H2 en modo PostgreSQL (`MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1`); en desarrollo se usa `jdbc:h2:mem:arume`
 - **Migraciones**: Flyway Community con historiales independientes. Los nombres de las tablas de historial de Flyway empiezan por `_` (p. ej. `_flyway_core_schema_history`, `_flyway_es_schema_history`, `_flyway_uk_schema_history`) para que queden agrupadas aparte de las tablas de negocio en el navegador de la base de datos; un `_` inicial es un identificador SQL válido en H2/PostgreSQL y Flyway lo acepta tal cual. Renombrar una tabla de historial hace que Flyway vea la base como sin migrar y reaplique todo, así que en desarrollo (base reiniciada) no hay problema, pero en una base ya poblada hay que renombrar la tabla manualmente o hacer `baseline`. Las migraciones core están en `arume-db/src/main/resources/db/migration/core/`, las españolas en `arume-es/src/main/resources/db/migration/es/` y las británicas en `arume-uk/src/main/resources/db/migration/uk/`; `arume-app` compone su ejecución en orden core → país. Mientras el producto no tenga instalaciones que preservar, cada módulo mantiene **una única
...
- **Tipo de sujeto de empresa**: el producto **ya no modela tipo de sujeto** (persona física/jurídica) como parte de la empresa; el enum de dominio `SubjectType` fue eliminado. La empresa se identifica únicamente por su identificación fiscal y su forma jurídica (código dentro de la jurisdicción).
- **Catálogo de formas jurídicas**: es **único y de core** (`t5_legal_forms`), con **PK doble `(country_alpha2_code, code)`** (el código es único dentro de su jurisdicción) y FK a `t1_countries`. Cada fila lleva la columna informativa **`is_organization BOOLEAN NOT NULL`** (familia *individual* / *organización*) que **no** forma parte de la PK ni de ninguna FK; se usa solo para que la UI agrupe y filtre sin recorrer toda la lista. Ningún módulo nacional crea tabla propia: cada país solo inserta sus filas desde su migración (`es/V0.1.0.0` siembra 17 formas ES, `uk/V0.1.0.0` siembra 7 formas GB). Se lee mediante el puerto core `LegalFormFacade` y `LegalFormCatalogService`, **resoluble por jurisdicción sin filtro obligatorio por tipo de sujeto**; cada forma devuelta indica su familia. `t6_companies` declara una FK `(legal_form_jurisdiction, legal_form_code)` hacia el catálogo, de modo que la BBDD garantiza que la forma existe en la jurisdicción indicada. La UI usa un `ComboBox` de países (nombre localizado) y un filtro de familia (`LegalFormFamily`: *Organisation* / *Individual*) que condiciona el combo de formas; una jurisdicción sin catálogo deshabilita el combo.
