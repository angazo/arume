...
#### Scenario: Config file contains database settings
- **WHEN** `arume.yml` is written
- **THEN** it SHALL contain `arume.db.type` with the database type identifier and `spring.datasource.*` properties with url, driver-class-name, username, and password
...
- **WHEN** the user completes the wizard with valid input and clicks "Guardar"
- **THEN** the system SHALL write `arume.yml` with the selected database type, storage path, username, and password
