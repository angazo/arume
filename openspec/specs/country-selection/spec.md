...
Capture the user's country during the first-run wizard, persist it in `arume.yml`, and display a non-interactive flag indicator in the main
window title bar. The country is decoupled from the UI language: the user may choose any supported country and any supported language
independently.
